...
def connect_to_ns(count_retry=1, max_retry=3):
  global CONN
  print("Attempting to connect to {}:{}".format(NS_IP, NS_PORT))
  ...
  def do_upload(self, args):
    ...
    if args:
        ...
        if not os.path.exists(local_path):
            result["status"] = 0
            msg.append('No such resource in local path!')
        ...
        default_value = 4096
        block_size = int(res["nsconfig"].get('block_size', default_value))
        ...
        print('File splitted into {} blocks.'.format(blocks_len))
        done_size = 0
        with open(local_path, 'rb') as lf:
            ...
            while buff:
                ...
                target_ss = random.sample(storage_servers,
                                          replication_factor) if len(storage_servers) > replication_factor else storage_servers
  ...
  def do_download(self, args):
    ...
    if args:
      ...
      if result["status"]==0:
        ...
        self.print_response(result)
        return'''
      if not blocks or all(len(block) == 0 for block in blocks):
        result["status"] = 0
        result["message"] = "No file/empty file in the given remote directory."
        self.print_response(result)
        return
